...
| `UPLOAD_ARTIFACTS` | `false` | If set to `true`, will upload the generated `__site` folder to github artifacts so it can be used in a separate job. This can be useful if you set `DEPLOY` to `false` and want to do the deployment manually in a separate job<sup>1</sup>. | {`false`, `true`} |
...
<sup>1</sup>. This can be useful if you want to inspect the generated site or if your website generation must happen on a windows runner (e.g. if it relies on a package that only works well on windows) as the [deploy action](https://github.com/JamesIves/github-pages-deploy-action) doesn't work on windows runners (it uses `rsync`). In that case you would set `DEPLOY` to `false`, `UPLOAD_ARTIFACTS` to `true`, and in a separate job with a linux runner you would then download the artifact and deploy it.
